...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function convertTemperature() {
        ...
        temps.forEach(temp => {
            ...
            temp.innerText = `${value.toFixed(1)}${selectedValue}`;
        });
    }
   
    // Attach functions to buttons
    document.querySelector('.city[onclick="north_PS()"]').onclick = north_PS;
    document.querySelector('.city[onclick="jeruslem_PS()"]').onclick = jeruslem_PS;
    document.querySelector('.city[onclick="south_PS()"]').onclick = south_PS;
});
